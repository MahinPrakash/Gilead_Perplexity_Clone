...
if send_button==True:
    ...
    class list_output(BaseModel):
     result:list=Field(description='A list of 3 paraphrased search queries')


    text_to_searchquery_prompt=ChatPromptTemplate.from_messages([
        ('system','''Please rephrase the following prompt into three concise search queries that preserve its original meaning:{question}\n

        Example:

        i) User's Prompt: "What are the health benefits of regular exercise?"

        Paraphrased Versions:

        "Can you list the advantages of engaging in consistent physical activity for health?"

        "What positive effects does regular exercise have on one's health?"

        "How does maintaining a routine of exercise benefit overall health?"

        ii) User's Prompt: "What is the latest US election results?"

        Paraphrased Versions:

        "Who won the latest US election results?"

        "What are the current outcomes of the latest US election?"

        "Share the latest election results in the United States?"
